...
PID myPID_left(&Input_left, &Output_left, &Setpoint_left,0.5,5,0, DIRECT);
double Setpoint_right, Input_right, Output_right;
PID myPID_right(&Input_right, &Output_right, &Setpoint_right,0.5,5,0, DIRECT);
...
ros::Publisher chatterleft("chatterleft", &speedleft);
ros::Publisher chatterright("chatterright", &speedright);
ros::NodeHandle nh;
...
attachInterrupt(1,getplus_right,RISING);//Q1
 ...
 nh.advertise(chatterleft);
 nh.advertise(chatterright);
}
...
Input_right = speed_right;
     ...
     speedleft.data = speed_left;
     chatterleft.publish( &speedleft );
     speedright.data = speed_right;
     chatterright.publish( &speedright );
     nh.spinOnce();
     delay(100);
}
void get_speed()
